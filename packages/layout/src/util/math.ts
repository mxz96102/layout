import { Matrix, Model, IndexMap, Edge, OutNode } from '../layouts/types';
...
export const getNodeDegreeMap = (edges: Edge[]) => {
  const nodeDegreeMap: { [key: string]: number } = {};
...
export const sum = (arr: number[], end?: number) => {
  let i = end || arr.length;
  let sum = 0;
