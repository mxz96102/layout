...
import { Edge, OutNode, NodeSize, PointTuple, Point, Node } from '../types';
import {Layout} from '../Layout';
import { isArray, isNumber, isObject, getEdgeTerminal, getFunc, isString, getNodeSize } from '../../util';
...
export default class DagreLayout extends Layout {
  option: DagreLayoutOptions ={...defaultDagreLayoutOptions}

  getDefaultCfg() {
    return { ...defaultDagreLayoutOptions };
  }
  ...
  public execute() {
    ...
    if (!nodeSize) {
      ...
      nodeSizeFunc = (node) => getNodeSize(nodeSize, node);
    }
    const ranksepfunc = getFunc(ranksep, 50, ranksepFunc);
    const nodesepfunc = getFunc(nodesep, 50, nodesepFunc);
    ...
    const nodeLayoutMap: { [key: string]: boolean } = {};
    ...
    nodes
      ...
      .forEach((node) => {
        ...
        if (isNumber(layer)) {
          ...
          g.setNode(node.id, { width, height, layer: Number(layer) });
        } else {
          g.setNode(node.id, { width, height });
        }

        if (node.layout) {
          nodeLayoutMap[node.id] = true;
        }

        if (sortByCombo && node.comboId) {
    ...
    dagre.layout(g, {
      ...
      keepNodeOrder: Boolean(nodeOrder),
      nodeOrder: nodeOrder,
    });
    ...
    if (radial) {
      ...
      g.edges().forEach((edge: any) => {
        ...
        if (edgeLabelSpace && controlPoints && edges[i].type !== 'loop') {
