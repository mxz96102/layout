...
export default class RadialNonoverlapForce {
  ...
  public layout(): PointTuple[] {
    ...
    const maxDisplace = this.width / 10;
  ...
  private getRepulsion() {
    ...
    positions.forEach((v: PointTuple, i: number) => {
      ...
      positions.forEach((u: PointTuple, j: number) => {
        ...
        if (vecLength < this.nodeSizeFunc(nodes[i]) / 2 + this.nodeSizeFunc(nodes[j]) / 2) {
  ...
  private updatePositions() {
    ...
    const strictRadial = this.strictRadial;
    const f = this.focusID;
    const maxDisplace = this.maxDisplace || this.width / 10;
