...
import { getNodeSize, isArray } from '../util';
import { Layout } from './Layout';
import { Combo, NodeSize, OutNode, Point, PointTuple } from './types';
...
export type DagreCompoundLayoutOptions = {
  rankdir?: 'TB' | 'BT' | 'LR' | 'RL';
  align?: 'UL' | 'UR' | 'DL' | 'DR';
  begin?: PointTuple;
  nodeSize?: NodeSize;
...
export default class DagreCompoundLayout extends Layout<ModelType> {
  option: DagreCompoundLayoutOptions = {
    rankdir: 'TB',
    nodesep: 50,
    edgesep: 5,
    ranksep: 50,
  ...
  public init(data: ModelType) {
    ...
    // 确保此次排序按照用户输入顺序
    this.data.nodes = this.getDataByOrder((data.nodes || []).concat(hiddenNodes));
    this.data.edges = this.getDataByOrder((data.edges || []).concat(hiddenEdges));
    this.data.combos = (data.combos || []).concat(hiddenCombos.map((hc) => ({ ...hc, collapsed: true })));
  }
  ...
  public execute() {
    ...
    const { graphDef, graphOption, graphSettings } = this.getLayoutConfig();
  ...
  private getLayoutConfig(): {
    ...
    const { nodes, edges, combos } = this.data;
    const { nodeSize, rankdir, align, edgesep, nodesep, ranksep, settings } = this.option;
    ...
    /** 计算 nodeSize */
    const nodeSizeFunc = (node) => getNodeSize(nodeSize, node);
    ...
    // 初始化 padding
    const subSceneMeta = this.graphSettings?.subScene?.meta;
  ...
  private updateNodePosition(
    ...
    const {combos, nodes, edges} = this.data;
    const {  anchorPoint } = this.option;
  ...
  private updateEdgePosition(
    ...
    const { combos, edges } = this.data;
    const { controlPoints } = this.option;
    const dBegin = this.getBegin(flattenedNodes, flattenedEdges);
    ...
    if (controlPoints) {
      ...
      edges.forEach((sourceEdge) => {
        ...
        if (sourceNode && targetNode) {
          ...
          const sourceNodePath = this.getNodePath(sourceEdge.source);
          const targetNodePath = this.getNodePath(sourceEdge.target);
