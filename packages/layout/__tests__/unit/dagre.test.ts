...
describe('#DagreLayout', () => {
  ...
  it('dagre with array nodeSize', () => {
    ...
    const nodeSize: [number, number] = [100, 50];
    const dagre = new DagreLayout({
      controlPoints: false,
      nodeSize,
      nodesepFunc: () => {
    ...
    expect(node.x).toBeCloseTo(60);
    expect(node.y).toBeCloseTo(215);
  ...
  it('dagre with array size in node data', () => {
    ...
    expect(node.x).toBeCloseTo(350);
    expect(node.y).toBeCloseTo(85);
  ...
  it('dagre sortByCombo', () => {
    ...
    expect(node.x).toBeCloseTo(425);
    expect(node.y).toBeCloseTo(330);
    expect(edge.controlPoints).toEqual(undefined);
  });
})